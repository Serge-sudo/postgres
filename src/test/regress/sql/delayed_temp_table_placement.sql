...
--
-- This test exercises the delayed_temp_table_placement feature which delays
...
SELECT name, setting, context, short_desc FROM pg_settings WHERE name = 'delayed_temp_table_placement';

-- Test that the setting cannot be changed without restart
SET delayed_temp_table_placement = on;  -- should fail
RESET delayed_temp_table_placement;  -- should fail

-- ALTER SYSTEM should work for POSTMASTER parameters
ALTER SYSTEM SET delayed_temp_table_placement = on;
ALTER SYSTEM RESET delayed_temp_table_placement;
...
-- regardless of delayed_temp_table_placement setting
...
SELECT 
    ...
    short_desc
FROM pg_settings 
WHERE name = 'delayed_temp_table_placement';

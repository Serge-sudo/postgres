...
my $pgb_handle1 = $node1->pgbench("-n -c 1 -t $transactions -f $q1, 'postgres'", 0, [], [], 'concurrent INSERTs and CIC');
my $pgb_handle2 = $node2->pgbench("-n -c 20 -t $transactions -f $q2, 'postgres'", 0, [], [], 'concurrent INSERTs and CIC');

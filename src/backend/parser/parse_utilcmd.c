...
/*
 * transformShardOfPartition -
 *	  Handle CREATE TABLE ... SHARD OF syntax for sharded partitions
 *
 * Determines the target shard member using consistent hashing and returns
 * either a CreateStmt (if target is local) or CreateForeignTableStmt (if remote).
 * This allows the partition to be created correctly on first attempt without
 * needing conversion or duplicate creation.
 */
Node *
transformShardOfPartition(RangeVar *relation, RangeVar *parent,
						  ...
						  bool if_not_exists)
{
	...
	Oid			sgid;
	Oid			target_member;
	Oid			local_server_oid;
	Relation	parent_rel;
	char	   *partition_name;
	char	   *cluster_name_local;

	/* Look up the parent table */
	parent_oid = RangeVarGetRelid(parent, NoLock, false);
	parent_rel = table_open(parent_oid, AccessShareLock);

	/* Get the shard group ID from the parent table */
	...
	if (!OidIsValid(sgid))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
				 ...
				 errhint("Use PARTITION OF for non-sharded tables.")));

	/* Build the full partition name */
	partition_name = relation->relname;

	/* Determine target member using consistent hashing */
	target_member = GetPartitionTargetMember(sgid, partition_name, InvalidOid);
	...
	/* Get local server OID by matching cluster_name */
	cluster_name_local = GetConfigOption("cluster_name", false, false);
	if (cluster_name_local == NULL || cluster_name_local[0] == '\0')
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("cluster_name must be set to use SHARD OF syntax")));

	local_server_oid = get_foreign_server_oid(cluster_name_local, true);

	table_close(parent_rel, AccessShareLock);

	/* Check if target is local or remote */
	if (target_member == local_server_oid)
	{
		/* Target is local - create regular partition */
		CreateStmt *stmt = makeNode(CreateStmt);

		relation->relpersistence = relpersistence;
		stmt->relation = relation;
		stmt->tableElts = tableElts;
		stmt->inhRelations = list_make1(parent);
		stmt->partbound = partbound;
		stmt->partspec = partspec;
		stmt->ofTypename = NULL;
		stmt->constraints = NIL;
		stmt->accessMethod = accessMethod;
		stmt->options = options;
		stmt->oncommit = oncommit;
		stmt->tablespacename = tablespacename;
		stmt->if_not_exists = if_not_exists;
		stmt->is_distributed = false;
		stmt->is_worldwide = false;
		stmt->shardgroup = NULL;
	...
	else
	{
		...
		CreateStmt *base = makeNode(CreateStmt);
		char	   *server_name;
		DefElem    *schema_opt;
		DefElem    *table_opt;
		HeapTuple	srvtuple;
		...
		/* Get server name from target member OID */
		srvtuple = SearchSysCache1(FOREIGNSERVEROID, ObjectIdGetDatum(target_member));
		if (!HeapTupleIsValid(srvtuple))
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_OBJECT),
					 errmsg("foreign server with OID %u does not exist", target_member)));
		srvform = (Form_pg_foreign_server) GETSTRUCT(srvtuple);
		server_name = pstrdup(NameStr(srvform->srvname));
		ReleaseSysCache(srvtuple);

		/* Set up base CreateStmt fields */
		relation->relpersistence = relpersistence;
		base->relation = relation;
		base->tableElts = tableElts;
		base->inhRelations = list_make1(parent);
		base->partbound = partbound;
		base->partspec = partspec;
		base->ofTypename = NULL;
		base->constraints = NIL;
		base->accessMethod = NULL;  /* Foreign tables don't have access method */
		base->options = NIL;  /* Will use OPTIONS clause instead */
		base->oncommit = oncommit;
		base->tablespacename = NULL;  /* Foreign tables don't use tablespace */
		base->if_not_exists = if_not_exists;
		base->is_distributed = false;
		base->is_worldwide = false;
		base->shardgroup = NULL;

		/* Create OPTIONS for postgres_fdw */
		schema_opt = makeDefElem("schema_name",
								 (Node *) makeString(parent->schemaname ? parent->schemaname : "public"),
								 -1);
		table_opt = makeDefElem("table_name",
								(Node *) makeString(partition_name),
								-1);

		/* Set up CreateForeignTableStmt */
		stmt->base = *base;
		stmt->servername = server_name;
		stmt->options = list_make2(schema_opt, table_opt);

...
static void ReplicateCreateIndexToShardMembers(Oid tableId, Oid indexRelationId, Oid parentIndexId);
			...
			bool quiet)
{
	 ...
	 */
	if (concurrent && OidIsValid(rel->rd_rel->relsgid))
	{
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("CREATE INDEX CONCURRENTLY is not supported on shard group tables"),
				 errhint("Remove CONCURRENTLY option when creating indexes on distributed or worldwide tables.")));
	}
	...
	if (partitioned)
	{
		...
		/* Replicate index creation to shard members if needed */
		ReplicateCreateIndexToShardMembers(tableId, indexRelationId, parentIndexId);
...
ReindexIndex(const ReindexStmt *stmt, const ReindexParams *params, bool isTopLevel)
{
	...
	/*
	 * Check if index belongs to a shard group table and reject CONCURRENTLY option
	 */
	{
		HeapTuple	tuple;
		...
		if (HeapTupleIsValid(tuple))
		{
			...
			tableOid = IndexGetRelation(indOid, false);
			ReleaseSysCache(tuple);
			
			/* Now check the table's shard group */
			...
			if (HeapTupleIsValid(tuple))
			{
				...
				if ((params->options & REINDEXOPT_CONCURRENTLY) != 0 && 
					OidIsValid(classForm->relsgid))
				{
					ReleaseSysCache(tuple);
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							 errmsg("REINDEX CONCURRENTLY is not supported on indexes of shard group tables"),
							 errhint("Remove CONCURRENTLY option when reindexing indexes on distributed or worldwide tables.")));
				}
	...
	if (relkind == RELKIND_PARTITIONED_INDEX && !executing_remote_ddl)
	{
		...
		if (HeapTupleIsValid(tuple))
		{
			...
			if (OidIsValid(sgid))
			{
				char	   *indexname = get_rel_name(indOid);
				char	   *nspname = get_namespace_name(get_rel_namespace(indOid));
				List	   *members;
				ListCell   *lc;
				StringInfoData reindex_sql;
				
				/* Null checks for name lookups */
				if (indexname == NULL)
				{
					ReleaseSysCache(tuple);
				...
				{
					ReleaseSysCache(tuple);
					elog(ERROR, "cache lookup failed for namespace %u", get_rel_namespace(indOid));
				}
				
				members = get_shardgroup_members(sgid);
				
				initStringInfo(&reindex_sql);
				appendStringInfo(&reindex_sql, "REINDEX INDEX %s.%s",
								 quote_identifier(nspname),
								 quote_identifier(indexname));
				
				ereport(DEBUG1,
						(errmsg("replicating REINDEX INDEX to shard members"),
						 errdetail("Index: %s.%s, Shard Group: %u",
								   nspname, indexname, sgid)));
				
				/* Execute REINDEX on all shard members */
				foreach(lc, members)
				{
					...
					{
						ereport(DEBUG1,
								(errmsg("executing REINDEX on shard member \"%s\"",
										server->servername)));
						
						fdwroutine->ExecForeignDDL(serveroid, reindex_sql.data);
					}
					else
					{
						ereport(NOTICE,
								(errmsg("index should be reindexed on shard member \"%s\"",
										server->servername),
	 ...
	 */
	{
		...
		if (HeapTupleIsValid(tuple))
		{
				...
				OidIsValid(classForm->relsgid))
			{
				...
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("REINDEX CONCURRENTLY is not supported on shard group tables"),
						 errhint("Remove CONCURRENTLY option when reindexing distributed or worldwide tables.")));
			}
	...
	if (get_rel_relkind(heapOid) == RELKIND_PARTITIONED_TABLE && !executing_remote_ddl)
	{
		...
		if (HeapTupleIsValid(tuple))
		{
			...
			if (OidIsValid(sgid))
			{
				...
				/* This is REINDEX TABLE, always generate TABLE command */
				appendStringInfo(&reindex_sql, "REINDEX TABLE %s.%s",
								 quote_identifier(nspname),
								 quote_identifier(relname));
				
				ereport(DEBUG1,
						(errmsg("replicating REINDEX TABLE to shard members"),
				...
				foreach(lc, members)
				{
					...
					{
						ereport(NOTICE,
								(errmsg("indexes should be reindexed on shard member \"%s\"",
										server->servername),
								 errdetail("Execute: %s", reindex_sql.data),
								 errhint("The FDW for this server does not support automatic DDL execution.")));
					}
...
/*
 * ReplicateCreateIndexToShardMembers
 *		Replicate CREATE INDEX to all shard group members
 *
 * This function is called after an index is successfully created on the local server.
 * It replicates the index creation to all shard members if the table belongs to a
 * shard group. Skip replication if executing DDL from a remote server to prevent
 * infinite recursion.
 */
static void
ReplicateCreateIndexToShardMembers(Oid tableId, Oid indexRelationId, Oid parentIndexId)
{
	HeapTuple	tuple;
	Form_pg_class classForm;
	Oid			sgid;
	
	/* Skip if executing DDL from remote server (prevents infinite recursion) */
	if (executing_remote_ddl)
		return;
	
	/* Only replicate for top-level indexes (not partition indexes) */
	if (OidIsValid(parentIndexId))
		return;
	
	tuple = SearchSysCache1(RELOID, ObjectIdGetDatum(tableId));
	if (!HeapTupleIsValid(tuple))
		return;
	...
	if (OidIsValid(sgid))
	{
		...
		ereport(WARNING,
				(errmsg("replicating CREATE INDEX to shard members"),
				 errdetail("Table: %s.%s, Index OID: %u, Shard Group: %u",
						   nspname, relname, indexRelationId, sgid)));
		
		/* Execute CREATE INDEX on all shard members */
		foreach(lc, members)
		{
			Oid			serveroid = lfirst_oid(lc);
			...
			if (fdwroutine->ExecForeignDDL != NULL)
			{
				/* Get proper index definition using pg_get_indexdef */
				char *indexdef = pg_get_indexdef_string(indexRelationId);
				
				if (indexdef && strlen(indexdef) > 0)
				{
					ereport(DEBUG1,
							(errmsg("executing CREATE INDEX on shard member \"%s\"",
									server->servername),
							 errdetail("DDL: %s", indexdef)));
					
					fdwroutine->ExecForeignDDL(serveroid, indexdef);
					pfree(indexdef);
				}
				else
				{
					ereport(NOTICE,
							(errmsg("could not generate index definition for replication"),
							 errdetail("Index OID: %u, Table: %s.%s",
									   indexRelationId, nspname, relname),
							 errhint("The index may need to be created manually on shard member \"%s\".",
									 server->servername)));
				}
			}
			else
			{
				ereport(NOTICE,
						(errmsg("index should be created on shard member \"%s\"",
								server->servername),
						 errdetail("Create an index on table %s.%s manually.",
								   nspname, relname),
						 errhint("The FDW for this server does not support automatic DDL execution.")));
			}
		}

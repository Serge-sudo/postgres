			   ...
			   ObjectAddress *typaddress, const char *queryString)
{
	...
	/*
	 * Handle SHARD OF syntax: determine target member early using consistent hashing.
	 * If target member is remote, we need to execute a CREATE FOREIGN TABLE statement
	 * instead of creating a regular table locally.
	 */
	if (stmt->is_shard_partition && stmt->partbound)
	{
		Oid			parentId;
		Oid			sgid;
		...
		/* Get parent relation OID from inhRelations */
		if (list_length(stmt->inhRelations) != 1)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
					 errmsg("SHARD OF requires exactly one parent table")));
		
		/* We need to look up the parent relation to get its shard group */
		RangeVar *parent_rv = linitial_node(RangeVar, stmt->inhRelations);
		parentId = RangeVarGetRelid(parent_rv, AccessShareLock, false);
		parent_rel = table_open(parentId, AccessShareLock);
		
		/* Parent must have a shard group */
		...
		if (!OidIsValid(sgid))
		{
			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
					 errmsg("parent table must belong to a shard group")));
		}
		
		table_close(parent_rel, AccessShareLock);
		
		/* Determine target member using consistent hashing */
		relname_str = stmt->relation->relname;
		target_member_oid = GetPartitionTargetMember(sgid, relname_str, InvalidOid);
		
		if (!OidIsValid(target_member_oid))
			ereport(ERROR,
					(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					 errmsg("no suitable shard member found for partition")));
		
		/* Check if target is local or remote */
		/* Determine local server OID by finding the member whose name matches cluster_name */
		{
			...
			foreach(lc, members)
			{
				...
				ForeignServer *srv = GetForeignServer(serveroid);
				
				if (cluster_name && strcmp(srv->servername, cluster_name) == 0)
				{
					local_server_oid = serveroid;
					break;
				}
			}
			list_free(members);
			
			if (!OidIsValid(local_server_oid))
				ereport(ERROR,
						(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
						 errmsg("local server not found in shard group")));
			
			target_is_local = (target_member_oid == local_server_oid);
			
			if (!target_is_local)
			{
			/* 
			 * Target is remote - execute CREATE FOREIGN TABLE statement instead.
			 * Build the SQL string and execute it via SPI.
			 */
			StringInfoData sql;
			ForeignServer *server;
			char	   *schema_name;
			char	   *partition_bound_str;
			int			ret;
			
			/* Get server information */
			server = GetForeignServer(target_member_oid);
			
			/* Get schema name from the parent relation */
			schema_name = get_namespace_name(get_rel_namespace(parentId));
			
			/* Build partition bound string */
			partition_bound_str = nodeToString(stmt->partbound);
			
			/* Build CREATE FOREIGN TABLE statement */
			initStringInfo(&sql);
			appendStringInfo(&sql, "CREATE FOREIGN TABLE %s PARTITION OF %s ",
							 quote_identifier(relname_str),
							 quote_identifier(parent_rv->relname));
			
			/* Add partition bound */
			appendStringInfo(&sql, "%s ", partition_bound_str);
			
			/* Add SERVER clause */
			appendStringInfo(&sql, "SERVER %s ", quote_identifier(server->servername));
			
			/* Add OPTIONS clause for postgres_fdw */
			appendStringInfo(&sql, "OPTIONS (schema_name %s, table_name %s)",
			...
			if (ret != SPI_OK_UTILITY)
				elog(ERROR, "CREATE FOREIGN TABLE execution failed");
			
			SPI_finish();
			
			/* Get the OID of the created foreign table */
		...
		/* If we get here, target is local, proceed with normal table creation */
	}
	...
	else if (stmt->is_shard_partition && stmt->partbound != NULL && OidIsValid(rel->rd_rel->relsgid))
	{
		/*
		 * For SHARD OF partitions: target member already determined, local partition already created.
		 * Now create tables on remote shard members only (local server is skipped in CreateTablesOnShardMembers)
		 */
		Oid parentId = linitial_oid(inheritOids);
		CreateTablesOnShardMembers(relationId, rel->rd_rel->relsgid, true, parentId, stmt->partbound);
	}
							...
							Oid parentOid, PartitionBoundSpec *partbound)
{
	...
	StringInfoData create_foreign_table_sql;
	char	   *relname;
	char	   *nspname;
	int			i;
	int			first_col;

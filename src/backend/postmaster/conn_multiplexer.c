 ...
 * Start or retry transaction reservation for already buffered frontend data.

 * This is needed after TX_BEGIN_WAIT: no more data may arrive from the backend,

 * so relying only on the next readable backend event can stall forever.

 */

static bool
mux_channel_start_tx(int idx)
{
	MuxChannelSlot *ch = &mux_channels[idx];
	
	if (ch->state != MCH_READY || ch->c2r_len <= 0 || ch->pending_tx_end)
		return true;
	...
	for (i = 0; i < MUX_MAX_WORKERS; i++)
	{
		...
		if (w->worker_sock == PGINVALID_SOCKET)
		{
			if (free_slot < 0)
			{
				free_slot = i;
				/* check if can spawn */
				if (mux_n_workers < mux_worker_count)
					break;
			}
		...
		if (strcmp(w->database, database) == 0 &&
			...
			w->in_tx == false && 
			mux_n_workers >= mux_worker_count
			// && TODO not session var set
			)
		{
			w->connect_cnt++;
...
ConnMuxEventLoop(void)
{
	...
	for (;;)
	{
		...
		for (i = 0; i < MUX_MAX_CHANNELS; i++)
		{
			...
			if (ch->state == MCH_READY && ch->c2r_len > 0)
			{
				if (!mux_channel_start_tx(i))
				{
					mux_channel_close(i);
					continue;
				}
		...
		for (i = 0; i < MUX_MAX_CTRL_CONNS; i++)
		{
			...
			if (cc->state == MCC_IN_TX &&
				cc->worker_idx >= 0 &&
				cc->worker_idx < MUX_MAX_WORKERS &&
				mux_workers[widx].worker_sock != PGINVALID_SOCKET)
			{
				MuxWorkerSlot *w = &mux_workers[cc->worker_idx];

				/*
				* Add worker socket only for the ctrl connection that currently owns it.
				* Otherwise the same fd can be added twice to epoll, causing:
				* epoll_ctl() failed: File exists
				*/
				if (w->worker_sock != PGINVALID_SOCKET &&
					w->in_tx &&
					w->active_channel == cc->channel_id)
				{
					uint32 we = WL_SOCKET_READABLE;

					if (cc->c2w_len > 0)
						we |= WL_SOCKET_WRITEABLE;

					AddWaitEventToSet(wes, we, w->worker_sock,

...
getFdwXactIdentifier(FdwXactEntry *fdwent, TransactionId xid)
{
	...
	snprintf(buf, FDWXACT_ID_MAX_LEN, "fx_%ld_%u_%u", labs((long)rand() * (long)rand()),
			 xid, fdwent->umid);
...
FdwXactInsertEntry(TransactionId xid, FdwXactEntry *fdwent, char *identifier)
{
	...
	fdwxact->locking_backend = MyProcNumber;
	LWLockRelease(FdwXactLock);
			   ...
			   char *identifier)
{
	...
	fdwxact->insert_start_lsn = InvalidXLogRecPtr;
	fdwxact->insert_end_lsn = InvalidXLogRecPtr;
	fdwxact->locking_backend = INVALID_PROC_NUMBER;
	fdwxact->valid = false;
 ...
 * reading WAL.	 TransamVariables->nextXid has been set to one more than
...
XlogReadFdwXactData(XLogRecPtr lsn, char **buf, int *len)
{
	...
	char	   *errormsg;
	TimeLineID	save_currtli = GetWALInsertionTimeLine();
	 ...
	 */
	SetWALInsertionTimeLine(save_currtli);
...
get_fdwxact_with_check(TransactionId xid, Oid umid, bool check_twophase_xact)
{
	...
	if (fdwxact->locking_backend != INVALID_PROC_NUMBER)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
#define PG_PREPARED_FDWXACTS_COLS	7
	...
	for (int i = 0; i < FdwXactCtl->num_xacts; i++)
	{
		...
		if (fdwxact->locking_backend != INVALID_PROC_NUMBER)
		{
			PGPROC	   *locker = GetPGProcByNumber(fdwxact->locking_backend);

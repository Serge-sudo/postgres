...
static void AdvanceXLInsertBuffer(XLogRecPtr upto, TimeLineID tli,
								  bool opportunistic);
static void XLogWrite(XLogwrtRqst WriteRqst, TimeLineID tli, bool flexible, PGPROC_LIST *wake_pendingWriteWALElem);
 ...
 */
static void
XLogWrite(XLogwrtRqst WriteRqst, TimeLineID tli, bool flexible, PGPROC_LIST *wake_pendingWriteWALElem)
{
	bool		ispartialpage;
	bool		last_iteration;
	bool		finishing_seg;
	...
	while (LogwrtResult.Write < WriteRqst.Write)
	{
			...
			finishing_seg)
		{
			...
			if (finishing_seg)
			{
				 ...
				 * and flushed. Since XLogWrite may continue writing more data
				 * beyond this segment boundary, we can only wake processes whose
				 * writePos is <= the end of the segment we just wrote and flushed.
				 *
				 * We do this AFTER fsync to avoid having woken processes try to
				 * acquire WALFlushLock unnecessarily (they would see their data
				 * is already flushed and can proceed immediately).
				 *
				 * This is an optimization to reduce waiting time - processes whose
				 * data is complete don't need to wait for additional segments to
				 * be written.
				 */
				{
...
XLogFlush(XLogRecPtr record)
{
			...
			if (!proc->writeWAL)
			{
				...
				while (true)
				{
					...
					else
					{
						...
						XLogFsync(WriteRqst, insertTLI, false);
						goto wal_is_written;
					}
				}
			}
	...
	else
	{
		...
		XLogWrite(WriteRqst, insertTLI, false, wake_pendingWriteWALElem);
	}
...
XLogBackgroundFlush(void)
{
	...
	{
		XLogWrite(WriteRqst, insertTLI, flexible, NULL);
	}
	else
		LWLockRelease(WALWriteLock);

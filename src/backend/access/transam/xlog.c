...
StartupXLOG(void)
{
	...
	/*
	 * Reload shared-memory state for prepared and foreign
	 * prepared transactions.  This needs to
...
void
SetWALInsertionTimeLine(TimeLineID insertTLI)
{
	SpinLockAcquire(&XLogCtl->info_lck);
	XLogCtl->InsertTimeLineID = insertTLI;
	SpinLockRelease(&XLogCtl->info_lck);
}

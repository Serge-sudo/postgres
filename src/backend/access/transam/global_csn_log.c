...
static bool GlobalCSNLogPagePrecedes(long page1, long page2);
						   ...
						   GlobalCSN csn, int pageno)
{
	...
	LWLock	   *lock = SimpleLruGetBankLock(GlobalCsnlogCtl, pageno);
...
GlobalCSNLogShmemInit(void)
{
	...
	SimpleLruInit(GlobalCsnlogCtl, "GlobalCSNLog Ctl", GlobalCSNLogShmemBuffers(), 0,
				"pg_global_csn",  LWTRANCHE_GLOBAL_CSN_LOG_BUFFERS, LWTRANCHE_GLOBAL_CSN_LOG_SLRU,  SYNC_HANDLER_NONE, false);
}
...
{
	int			slotno;
	LWLock	   *lock = SimpleLruGetBankLock(GlobalCsnlogCtl, 0);
...
StartupGlobalCSNLog(TransactionId oldestActiveXID)
{
	...
	for (;;)
	{
		lock = SimpleLruGetBankLock(GlobalCsnlogCtl, startPage);
		if (prevlock != lock)
		{
			if (prevlock)
...
ShutdownGlobalCSNLog(void)
{
	...
	SimpleLruWriteAll(GlobalCsnlogCtl, false);
...
CheckPointGlobalCSNLog(void)
{
	...
	SimpleLruWriteAll(GlobalCsnlogCtl, true);
...
ExtendGlobalCSNLog(TransactionId newestXact)
{
	...
	lock = SimpleLruGetBankLock(GlobalCsnlogCtl, pageno);
 ...
 */
static bool
GlobalCSNLogPagePrecedes(long page1, long page2)
{
	TransactionId xid1;
	TransactionId xid2;
